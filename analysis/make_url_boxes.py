...
query = """
        SELECT VisualComplete85,url   FROM results WHERE step == 0 AND location LIKE '%tor-without-timer%';
"""
...
seriesVals = sorted(list(series.items()),key=lambda x : np.median(x[1]))
...
fig.update_xaxes(tickangle=90, tickfont=dict(size=25))
